...
describe('IngredientDrawSHyperCutter', () => {
  ...
  it('should have correct basic properties', () => {
    expect(IngredientDrawSHyperCutter.name).toBe('Hyper Cutter (Ingredient Draw S)');
    expect(IngredientDrawSHyperCutter.description({ skillLevel: 1 })).toBe(
  ...
  it('should calculate correct ingredient amounts', () => {
    expect(IngredientDrawSHyperCutter.activations.ingredients.amount({ skillLevel: 1 })).toBe(5);
    expect(IngredientDrawSHyperCutter.activations.ingredients.amount({ skillLevel: 4 })).toBe(11);
    expect(IngredientDrawSHyperCutter.activations.ingredients.amount({ skillLevel: 7 })).toBe(18);
  });

  it('should calculate correct ingredient crit amounts', () => {
    expect(IngredientDrawSHyperCutter.activations.ingredients.critAmount!({ skillLevel: 1 })).toBe(10);
    expect(IngredientDrawSHyperCutter.activations.ingredients.critAmount!({ skillLevel: 4 })).toBe(22);
    expect(IngredientDrawSHyperCutter.activations.ingredients.critAmount!({ skillLevel: 7 })).toBe(36);
  });
