...
describe('evolvesFrom', () => {
  it('shall have 1 fewer remaining evolution', () => {
    expect(evolvesFrom(MOCK_POKEMON, 'Mock').remainingEvolutions).toBe(0);
  });

  it('shall have 1 more previous evolution', () => {
    expect(evolvesFrom(MOCK_POKEMON, 'Mock').previousEvolutions).toBe(2);
  });
});

describe('evolvesInto', () => {
  it('shall have 1 more remaining evolution', () => {
    expect(evolvesInto(MOCK_POKEMON, 'Mock').remainingEvolutions).toBe(2);
  });

  it('shall have 1 fewer previous evolution', () => {
    expect(evolvesInto(MOCK_POKEMON, 'Mock').previousEvolutions).toBe(0);
  });
});
