...
describe('SkillState', () => {
  ...
  it('should instantiate pityProcThreshold', () => {
    expect(skillState['memberState'].member.settings.pityProcThreshold).toBeGreaterThan(0);
  });
  ...
  it("should guarantee skill activation if we're past the pity threshold", () => {
    ...
    skillState['helpsSinceLastSkillProc'] = skillState['memberState'].member.settings.pityProcThreshold;
  ...
  it('should activate skill if roll is successful', () => {
    ...
    skillState['helpsSinceLastSkillProc'] = skillState['memberState'].member.settings.pityProcThreshold - 1; // ensure we don't hit pity threshold
    const activation1 = skillState.attemptSkill();
    const activation2 = skillState.attemptSkill();
    expect([activation1, activation2]).toHaveLength(2);
  });
  ...
  it('should add bonus activations correctly', () => {
    ...
    expect(activation).toBeDefined();
    expect(skillState['skillProcs']).toBe(1);
    expect(skillState['helpsSinceLastSkillProc']).toBe(skillState['memberState'].member.settings.pityProcThreshold);
  });
