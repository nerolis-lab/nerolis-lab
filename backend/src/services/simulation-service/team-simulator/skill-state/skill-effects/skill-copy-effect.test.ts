...
describe('SkillCopyEffect', () => {
  ...
  it('should activate the copied skill', () => {
    ...
    skillState.skillEffects.set(skillState.memberState.otherMembers[0].pokemonWithIngredients.pokemon.skill, {
    ...
    expect(
      skillState.skillEffects.get(skillState.memberState.otherMembers[0].pokemonWithIngredients.pokemon.skill)?.activate
    ).toHaveBeenCalledWith(skillState);
  });

  it('should fallback to CHARGE_STRENGTH_S if copied skill is same or modified version of SKILL_COPY', () => {
    skillState.memberState.otherMembers[0].pokemonWithIngredients.pokemon.skill.isSameOrModifiedVersion = vi
      .fn()
  ...
  it('should log an error if activation fails', () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(`[${skillState.skill.name}] Couldn't activate Test Skill`);
  });

  it('should fallback to CHARGE_STRENGTH_S if no skill is copied', () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    skillState.memberState.otherMembers[0].pokemonWithIngredients.pokemon.skill = null as any;
    const mockActivation = { skill: skillState.skill };
    skillState.skillEffects.set(mainskill.CHARGE_STRENGTH_S, {
      activate: vi.fn().mockReturnValue(mockActivation)
    });
