...
// Initialize with a single database that we'll manage manually
...
describe('Migration 012_team_area', () => {
  ...
  afterEach(async () => {
    ...
    // Migrate down to reset for next test
  ...
  it('should migrate team favored_berries to team_area and user_area', async () => {
    ...
    const testTeams = [
      ...
      },
      {
        ...
        favored_berries: null, // Greengrass teams would have null/empty berries
        version: 1,
        camp: false,
        team_index: 2,
        bedtime: '21:30',
    ...
    expect(teamAreaBerries).toContain(null); // Greengrass team
  ...
  it('should handle null/empty berry combinations as greengrass', async () => {
    // Insert test data
    const testUserId = 1;
    await knex('user').insert({
      id: testUserId,
    ...
    const testTeam = {
      ...
      fk_user_id: testUserId,
      name: 'Team Greengrass',
      favored_berries: null, // Greengrass teams would have null berries
      version: 1,
      camp: false,
      team_index: 0,
      bedtime: '21:30',
    ...
    expect(teamArea.favored_berries).toBe(null); // Should preserve the null value
