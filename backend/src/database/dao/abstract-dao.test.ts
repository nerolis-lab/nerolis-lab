...
describe('AbstractDAO', () => {
  ...
  describe('basic DAO operations', () => {
    ...
    it('should findOrInsert - insert when entity does not exist', async () => {
      const result = await testDAO.findOrInsert({ name: 'not-existing' }, { name: 'not-existing', value: 999 });
    ...
    it('should findOrInsert - return existing entity when found', async () => {
      ...
      const result = await testDAO.findOrInsert(
        ...
        { name: 'existing-entity', value: 456 } // Different value should be ignored
      );
    ...
    it('should findOrInsert with complex filter', async () => {
      ...
      const result1 = await testDAO.findOrInsert({ value: 100 }, { name: 'should-not-insert', value: 300 });

      expect(result1.name).toBe('complex1');
      expect(result1.value).toBe(100);

      // Insert new with value filter that doesn't match
      const result2 = await testDAO.findOrInsert({ value: 999 }, { name: 'new-complex', value: 999 });
  ...
  describe('transaction parameter support', () => {
    it('should accept transaction parameter in all methods', async () => {
      // This test verifies that all methods accept transaction parameters
      // without actually testing transaction isolation (which is complex with SQLite)

      const knex = await DatabaseService.getKnex();

      // Create a mock transaction that behaves like a regular connection
      const mockTrx = knex as TransactionContext;

      // Test all methods accept transaction parameter without throwing
      const inserted = await testDAO.insert({ name: 'tx-test', value: 1 }, mockTrx);
      expect(inserted.name).toBe('tx-test');

      const found = await testDAO.find({ name: 'tx-test' }, { trx: mockTrx });
      expect(found).toBeDefined();

      const updated = await testDAO.update({ ...inserted, value: 2 }, mockTrx);
      expect(updated.value).toBe(2);

      const multiple = await testDAO.findMultiple({}, { trx: mockTrx });
      expect(multiple.length).toBeGreaterThan(0);

      const count = await testDAO.count({}, mockTrx);
      ...
      const foundOrInserted = await testDAO.findOrInsert(
        { name: 'find-or-insert-test' },
        { name: 'find-or-insert-test', value: 77 },
        mockTrx
      );
      expect(foundOrInserted.name).toBe('find-or-insert-test');

      // Clean up
      await testDAO.delete({ name: 'tx-test' }, mockTrx);
      await testDAO.delete({ name: 'batch1' }, mockTrx);
      await testDAO.delete({ name: 'batch2' }, mockTrx);
      await testDAO.delete({ name: 'upsert-test' }, mockTrx);
      await testDAO.delete({ name: 'find-or-insert-test' }, mockTrx);
    });
  ...
  describe('DatabaseService.getKnex transaction handling', () => {
    it('should return transaction when provided', async () => {
      ...
      const mockTransaction = normalKnex as TransactionContext;
