...
import { DatabaseService, type TransactionContext } from '../database-service.js';
...
> {
  ...
  async find(
    ...
    options?: { sort?: SortKey<DBEntityType> | Array<SortKey<DBEntityType>>; trx?: TransactionContext }
  ...
  async insert(
    ...
    return this.get({ id: result[0] } as any, options);
  }

  async update(entity: DBEntityType, options?: { trx?: TransactionContext }): Promise<DBEntityType> {
    const knex = await DatabaseService.getKnex(options?.trx);
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return this.get({ id: entity.id } as any, options);
  }

  async delete(filter: Filter<DBEntityType> = {}, options?: { trx?: TransactionContext }): Promise<number> {
    const knex = await DatabaseService.getKnex(options?.trx);
  ...
  public async count(filter: Filter<DBEntityType> = {}, options?: { trx?: TransactionContext }) {
    const knex = await DatabaseService.getKnex(options?.trx);
