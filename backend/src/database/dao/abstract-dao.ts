...
> {
  ...
  protected preProcess<X extends object | undefined>(row: X): any {
    ...
    const autoGeneratedKeys = ['created_at', 'updated_at'];

    Object.entries(processed).forEach(([key, value]) => {
      if (value === undefined) {
        processed[key] = null;
      } else if (value instanceof Date && !autoGeneratedKeys.includes(key)) {
  ...
  protected postProcess<X extends object | undefined>(row: X): X {
    ...
    Object.entries(this.schema.properties).forEach(([key, value]) => {
      ...
      } else if (schema[Kind] === 'Timestamp' || schema[Kind] === 'DateTime') {
        if (typeof current === 'string' && !isNaN(Date.parse(current))) {
          result[key] = new Date(current);
        }
      }
    });
    return result;
