...
export default defineComponent({
  ...
  async mounted() {
    try {
      ...
      if (error) {
        logger.error(`Patreon authentication error: ${error}`)
        this.router.push('/')
        return
      }

      if (!code) {
        logger.error('No Patreon authorization code provided')
        this.router.push('/')
        return
      }
      ...
      logger.error(`Patreon login failed: ${error instanceof Error ? error.message : String(error)}`)
