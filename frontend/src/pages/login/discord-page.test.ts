...
import DiscordPage from './discord-page.vue'
...
describe('DiscordPage', () => {
  let wrapper: VueWrapper<InstanceType<typeof DiscordPage>>
  let userStore: ReturnType<typeof useUserStore>

  const mountComponent = (query = {}) => {
    return mount(DiscordPage, {
      props: {
  ...
  it('should display loading message', () => {
    ...
    expect(wrapper.text()).toContain('Logging in with Discord...')
  ...
  it('should redirect to home if error is present in query', async () => {
    ...
    expect(global.logger.error).toHaveBeenCalledWith('Discord authentication error: access_denied')
  ...
  it('should redirect to home if no code is present', async () => {
    ...
    expect(global.logger.error).toHaveBeenCalledWith('No Discord authorization code provided')
    expect(useRouter().push).toHaveBeenCalledWith('/')
  })
