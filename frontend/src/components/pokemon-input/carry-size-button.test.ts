...
describe('CarrySizeButton', () => {
  ...
  beforeEach(() => {
    ...
    wrapper = mount(CarrySizeButton, {
      props: {
        memberIndex: 0,
        pokemonInstance: initialPokemon
      }
    })
  })
  ...
  it('updates carry limit and selected evolutions the first time a pokemon is set', async () => {
    ...
    expect(wrapper.emitted('update-carry')).toBeTruthy()
    expect(wrapper.emitted('update-carry')!.at(-1)).toEqual([20])
    expect(wrapper.vm.$data.selectedEvolutions).toBe(1)
  })

  it('updates carry limit to max possible when the pokemon is changed', async () => {
    await wrapper.setProps({
      pokemonInstance: {
        ...initialPokemon,
        pokemon: venusaur
    ...
    expect(wrapper.emitted('update-carry')).toBeTruthy()
    expect(wrapper.emitted('update-carry')!.at(-1)).toEqual([25])
  })
})
