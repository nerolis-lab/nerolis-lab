<template>
  <v-dialog v-model="dialogStore.slotActionsDialog" max-width="600px">
    <v-card v-if="dialogStore.slotActionsProps.pokemon" :loading="savedState.loading">
      <v-list>
        <div id="filledMenu">
          ...
          <v-list-item
            id="duplicateButton"
            :disabled="dialogStore.slotActionsProps.fullTeam"
...
watch(
  () => dialogStore.slotActionsProps.pokemon,
  (newPokemon) => {
    if (newPokemon) {
...
const handleEditClick = () => {
  const pokemon = dialogStore.slotActionsProps.pokemon
  const onUpdate = dialogStore.slotActionsProps.onUpdate
  if (!pokemon) return

  dialogStore.closeSlotActions()
...
const toggleSavedState = () => {
  ...
  if (savedState.value.state !== savedState.value.serverState) {
    ...
    if (dialogStore.slotActionsProps.onToggleSaved) {
      dialogStore.slotActionsProps.onToggleSaved(savedState.value.state)
    }
  }
}
