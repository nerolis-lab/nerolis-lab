    ...
    <v-col cols="auto" class="flex-center flex-nowrap mx-4">
      ...
      >
        <v-img
          ...
          height="40px"
          width="40px"
          :alt="`Charge Energy S level ${effectiveSkillLevel}`"
          title="Charge Energy S"
        ></v-img>
      </v-badge>
      <div class="ml-2">
...
export default defineComponent({
  ...
  computed: {
    ...
    energyPerProc() {
      return ChargeEnergyS.activations.energy.amount({ skillLevel: this.effectiveSkillLevel })
    },
    totalEnergy() {
      const strengthSkillValue = this.memberWithProduction.production.skillValue['energy'] ?? {
        amountToSelf: 0,
        amountToTeam: 0
      }
      return compactNumber((strengthSkillValue.amountToSelf + strengthSkillValue.amountToTeam) * this.timeWindowFactor)
    },
    timeWindowFactor() {
