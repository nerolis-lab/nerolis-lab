...
describe('MemberProductionSkill with a GGEX skill level bonus', () => {
  let wrapper: VueWrapper<InstanceType<typeof MemberProductionSkill>>

  const bonusedMember = mocks.createMockMemberProductionExt({
    member: mocks.createMockPokemon({ pokemon: BRAVIARY, skillLevel: 1 }),
    production: mocks.createMockMemberProduction({ skillLevel: 2 })
  })

  beforeEach(async () => {
    wrapper = mount(MemberProductionSkill, {
      props: {
        memberWithProduction: bonusedMember
      }
    })
    await flushPromises()
    await vi.dynamicImportSettled()
  ...
  it('displays the bonused skill level with a * indicator', () => {
    const skillLevelBadge = wrapper.find('#skillLevelBadge')
    expect(skillLevelBadge.text()).toBe('Lv.2*')
  })

  it('computes the per-activation skill value from the bonused skill level', () => {
    const skillValuePerProc = wrapper.find('.font-weight-light.text-body-2')
    expect(skillValuePerProc.text()).toBe(
      `x${bonusedMember.member.pokemon.skill.amount(bonusedMember.production.skillLevel)}`
    )
  })
})
