...
import { DRIFBLIM, MathUtils, compactNumber, localizeNumber } from 'sleepapi-common'
...
describe('MemberProductionSkill', () => {
  ...
  it('displays the correct skill value per proc, comma-formatted', () => {
    ...
    expect(skillValuePerProc.text()).toBe(`${localizeNumber(rawAmount)} avg.`)
  })
  ...
  it('applies the current team island area bonus to the skill value per proc', async () => {
    ...
    expect(skillValuePerProc.text()).toBe(`${localizeNumber(expectedValue)} avg.`)
  })
