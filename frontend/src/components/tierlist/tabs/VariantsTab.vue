        ...
        <v-col cols="12" sm="">
          <div class="d-flex align-center flex-wrap ga-1">
            <CustomChip
              ...
              :key="mIndex"
              size="small"
              :prepend-avatar="pokemonDisplayImageUrlByName(mate.name)"
              color="accent"
            >
              <v-avatar v-for="ing in mate.ingredientList" :key="ing.name" size="20" class="ingredient-avatar">
              ...
              <template #append>
                <v-avatar v-for="recipe in mate.recipe" :key="recipe" size="22" class="mr-1">
...
import { localizeNumber, type IngredientSetSimple, type PokemonWithTiering } from 'sleepapi-common'
...
function getSynergisticTeammates(variant: PokemonWithTiering) {
  const teammateCounts: Record<
    string,
    { name: string; ingredientList: IngredientSetSimple[]; count: number; recipe: string[] }
  > = {}
  variant.contributions?.forEach((contrib) => {
    contrib.team?.forEach((teamMember) => {
      if (
        // Exclude self from synergies
        !(
          teamMember.pokemon === pokemonName.value &&
          JSON.stringify(teamMember.ingredientList) === JSON.stringify(variant.pokemonWithSettings.ingredientList)
        )
      ) {
        if (!teammateCounts[teamMember.pokemon]) {
          teammateCounts[teamMember.pokemon] = {
            name: teamMember.pokemon,
        ...
        teammateCounts[teamMember.pokemon].recipe.push(contrib.recipe)
      }
    })
  })
  const sortedTeammates = Object.values(teammateCounts).sort((a, b) => b.count - a.count)

  if (sortedTeammates.length === 0) {
    return []
  }

  // Only return teammates with the highest count
  const maxCount = sortedTeammates[0].count
  return sortedTeammates.filter((mate) => mate.count === maxCount)
}
